...
*Setup instructions for a gitian build of Evos using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Evos
...
Clone the git repositories for evos and gitian and then checkout the evos version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the evos repository
...
To build Evos (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the evos repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/evos/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Evos
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Evos.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit evos=${COMMIT} --url evos=${URL} ../Evos/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit evos=${COMMIT} --url evos=${URL} ../Evos/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit evos=${COMMIT} --url evos=${URL} ../Evos/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/evos-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/evos-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/evos-build.assert
```
