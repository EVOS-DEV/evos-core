...
#define MASTERNODE_COLLATERAL_LIMIT_LOW (MASTERNODE_COLLATERAL - 0.01 * COIN)
#define MASTERNODE_COLLATERAL_HIGH (15000 * COIN)
#define MASTERNODE_COLLATERAL_LIMIT_HIGH (MASTERNODE_COLLATERAL - 0.01 * COIN)

#define MN_INCREASE_COLLATERAL 90000
#define MN_INCREASE_GRACEBLOCKS	1000

CAmount CMasternodeBroadcast::GetMasternodeCollateralToActivate(int nHeight)
{
	if (nHeight < MN_INCREASE_COLLATERAL)
		return MASTERNODE_COLLATERAL_LOW;
	else
		return MASTERNODE_COLLATERAL_HIGH;
}

CAmount CMasternodeBroadcast::GetMasternodeCollateralToSurvive(int nHeight)
{
	if (nHeight < MN_INCREASE_COLLATERAL+MN_INCREASE_GRACEBLOCKS)
		return MASTERNODE_COLLATERAL_LOW;
	else
		return MASTERNODE_COLLATERAL_HIGH;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 10000 EVOS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 EVOS tx -> 1 confirmation
