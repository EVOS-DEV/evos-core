...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::EVOS, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (evos.conf).");
            QMessageBox::warning(this, "Evos Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
void BlockExplorer::setContent(const std::string& Content)
{
    ...
    QString CSS = "body {font-size:12px; color:#229c86; bgcolor:#051512;}\n a, span { font-family: monospace; }\n span.addr {color:#229c86; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #051512;}\n td.d0 {font-weight: bold; color:#229c86;}\n h2, h3 { white-space:nowrap; color:#229c86;}\n a { color:#2ed1b3; text-decoration:none; }\n a.nav {color:#229c86;}\n";
    QString FullContent = "<html><head><style type=\"text/css\">" + CSS + "</style></head>" + "<body bgcolor=#051512>" + Content.c_str() + "</body></html>";
