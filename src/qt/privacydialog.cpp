                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEVOS ought to be enough for anybody." - Bill Gates, 2017
    ui->zEVOSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEVOSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEVOS->setEnabled(false);
        ui->pushButtonMintzEVOS->setToolTip(tr("zEVOS is currently disabled due to maintenance."));

        ui->pushButtonSpendzEVOS->setEnabled(false);
        ui->pushButtonSpendzEVOS->setToolTip(tr("zEVOS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEVOSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEVOS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEVOS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEVOS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEVOS in ") +
...
void PrivacyDialog::on_pushButtonSpendzEVOS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEVOS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEVOS
        sendzEVOS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEVOS
    sendzEVOS();
}

void PrivacyDialog::on_pushButtonZEvosControl_clicked()
{
    ZEvosControlDialog* zEvosControl = new ZEvosControlDialog(this);
    zEvosControl->setModel(walletModel);
    zEvosControl->exec();
}

void PrivacyDialog::setZEvosControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEvosSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEVOS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Evos Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEVOSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEVOS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EVOS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEVOS</b>";
    ...
    // use mints from zEvos selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEvosControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEvosControlDialog::GetSelectedMints();
    }
    ...
    // Clear zevos selector in case it was used
    ZEvosControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEvos Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Evos, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEvos Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEVOS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEVOS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEVOS "));
    ui->labelzEVOSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEVOSSyncStatus->setVisible(fShow);
}
