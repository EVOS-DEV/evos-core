...
#include "ui_zevoscontroldialog.h"
...
std::list<std::string> ZEvosControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEvosControlDialog::listMints;

ZEvosControlDialog::ZEvosControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEvosControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEvosControlDialog::~ZEvosControlDialog()
{
    delete ui;
}

void ZEvosControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEvosControlDialog::updateList()
{
...
void ZEvosControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEvosControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEvos_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEvosControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEvosControlDialog::GetSelectedMints()
{
...
void ZEvosControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
